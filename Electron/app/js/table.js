class Table {
  ...
  resize() {
    ...
    $(this.tableBody).height(
      $(this.table).height() - $(this.table.find("thead")[0]).height() + "px"
    );
  }
  ...
  load(content, type, listener) {
    ...
    if (!document.getElementById(type + "Error").classList.contains("error"))
      settings.current[type] = content;
