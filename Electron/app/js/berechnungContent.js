function loadTableCalculate() {
  new Table("berechnung", (type, self) => {
    ...
    $("#berechnungContentCalculate").click(() => {
      // Reset errors
      ...
      if (berufe.length > 1 && schüler.length > 1) {
        ...
        if (schüler[0].equals(berufe[0])) {
          for (var time = 0; time < schüler[0].length; time++) {
            for (var i = 1; i < schüler.length; i++) {
              try {
                ...
                if (schüler[i][time + 1])
                  data.schülerwünsche[schüler[0][time]][
                    schüler[i][time + 1]
                  ].push(schüler[i][0]);
              } catch (e) {
                errorList.push(
          ...
          errorList.push(
            "Fehler: Die Liste der Schüler ist nicht mit den Berufen kompatibel."
          );
        ...
        if (errorList.length) {
          ...
          errorList.forEach(
            (errorStudent) => (error.innerHTML += errorStudent + "\n")
          );
        } else {
          // Set initial wishes - cloning
          data.initialSchülerwünsche = JSON.parse(
            JSON.stringify(data.schülerwünsche)
          );
          ...
          // Show umverteilung
          $("#berechnungContentExtraButtons").css({
            display: "",
          });
        }
      } else {
    ...
    function schülerwünscheIntoTabe(schülerwünsche) {
      ...
      self.load(
        Object.entries(schülerwünsche).reduce(
          (current, [date, courses]) => {
            current[0].push(date);

            Object.entries(courses).forEach(([course, students], index) =>
              current[index + 1].push(
                course != "undefined"
                  ? course +
                      "<p style='color:" +
                      colorParticipants(data.min, data.max, students.length) +
                      ";' >" +
                      students.length +
                      "</p>"
                  : ""
          ...
          (() => {
            ...
            if (keys.length)
              return Object.keys(schülerwünsche[keys[0]]).reduce(
                (current) => {
                  current.push([]);
                  return current;
                },
        ...
        (event) => {
          event.path.some((element) => {
            ...
            if (element.tagName == "TD") {
              ...
              let date = Object.keys(data.initialSchülerwünsche)[
      ...
      Object.entries(initial).forEach(([time, students]) => {
        // Iterate students
        Object.entries(students).forEach(([student, initialWish]) => {
      ...
      Object.entries(allWishes).forEach(([student, wishes]) => {
      ...
      [
        ...
        var percentage = Math.round(
          (percentageValue.wrong / percentageValue.all) * 100
        );

        percentageObj.innerText = percentage + "%";
        percentageObj.style.color =
          "rgb(" +
          (255 * percentage) / 100 +
          "," +
          (255 - (255 * percentage) / 100) +
          ",0)";
      });
    }
    ...
    function colorParticipants(min, max, participants) {
      ...
      if (max && min) {
        if (
          participants >= max * data.ungenauigkeit ||
          participants * data.ungenauigkeit < min
        )
          return "red";
    ...
    $("#berechnungContentRedistribute").click(() => {
      // Reset data
      getUserData();
      ...
      let averageCourses = Object.fromEntries(
        Object.entries(data.courseCouples).map(([course, dates]) => {
          return [
            course,
            dates.reduce((current, date) => {
      ...
      getSizeCourses(data.schülerwünsche, averageCourses)
        .sort((a, b) =>
          ...
          sortByMinMaxPull(
            a.students,
            b.students,
            averageCourses[a.course],
            averageCourses[b.course]
          )
        )
    ...
    function sort(sizeCoursesSorted, averageCourses, studentWishes) {
      ...
      Object.entries(
        data.courseCouples[sizeCoursesSorted.course].reduce((current, date) => {
          // Remove courses which are blocked to sort into and are the same date
          if (
            date != sizeCoursesSorted.date &&
            (!data.blocked[date] ||
        ...
        .sort(
          ([courseA, studentsA], [courseB, studentsB]) =>
        ...
        .some(([orderedCourse]) => {
          ...
          return skipIteration(
            sizeCoursesSorted.date,
            sizeCoursesSorted.course,
            averageCourses,
            () => {
              // Seek for smallest courses to swap with
              Object.entries(
                Object.entries(
                  data.schülerwünsche[sizeCoursesSorted.date][
                    sizeCoursesSorted.course
                  ].reduce((current, student) => {
                    ...
                    if (
                      ...
                      studentWishes[orderedCourse][student] !=
                        sizeCoursesSorted.course &&
                      Object.keys(
                        data.schülerwünsche[sizeCoursesSorted.date]
                      ).includes(studentWishes[orderedCourse][student])
                    )
                      current[studentWishes[orderedCourse][student]] =
                        data.schülerwünsche[orderedCourse][
                          studentWishes[orderedCourse][student]
                        ].length;
                    return current;
                  }, {})
                  ...
                  .reduce((current, [course, students]) => {
                    ...
                    if (
                      !data.blocked[sizeCoursesSorted.date] ||
                ...
                .sort(([courseA, studentsA], [courseB, studentsB]) =>
                  ...
                  sortByMinMaxPull(
                    studentsA,
                    studentsB,
                    averageCourses[courseA],
                    averageCourses[courseB]
                  )
                )
                // Iterate swap courses
                .forEach(([orderedSwapCourse]) => {
                  // Iterate students out of orderedSwapCourses
                  data.schülerwünsche[orderedCourse][orderedSwapCourse].some(
                    (studentInSwapCourse) => {
                      ...
                      return skipIteration(
                        ...
                        () => {
                          ...
                          let cutIndex = data.schülerwünsche[
                            sizeCoursesSorted.date
                          ][sizeCoursesSorted.course].indexOf(
                            studentInSwapCourse
                          );
    ...
    function sortByMinMaxPull(a, b, averageA, averageB) {
      if (
        ...
        return (data.min || 0) - Math.min(a, b) >
          Math.max(a, b) - (data.max || Math.max(a, b))
          ? a - b
          : b - a;
    ...
    function skipIteration(date, course, averageCourses, worker) {
      ...
      if (data.min && data.max) {
        return (data.schülerwünsche[date][course].length < data.min &&
          data.schülerwünsche[date][course].length < averageCourses[course]) ||
          data.schülerwünsche[date][course].length > data.max
          ? worker()
          : true;
      }
      // Completely clear course if it is too small
        ...
        return data.schülerwünsche[date][course].length > data.max
          ? worker()
          : true;
      }
      // Remove students above average
      else {
        return data.schülerwünsche[date][course].length > averageCourses[course]
          ? worker()
          : true;
      }
    }
    ...
    function courseCouples(wishes) {
      return Object.entries(wishes).reduce((current, [date, courses]) => {
        // Add courses
        Object.entries(courses).forEach(([course, students]) => {
          current[course]
            ? current[course].push(date)
            : (current[course] = [date]);
        });
    ...
    function getStudentWishes(wishes) {
      ...
      return Object.fromEntries(
        Object.entries(wishes).map(([date, courses]) =>
          // Iterate courses in wish
          [
            date,
            Object.fromEntries(
              Object.entries(courses).reduce(
      ...
      for (var index = 1; index < initial.length; index++) {
        ...
        if (!initial[index][0]) {
          ...
          for (var date = 1; date < initial[index].length; date++) {
            // New course
            newData[dataIndex][index - subIndex][date] =
              wishes[initial[0][date]][initial[index][0]];
          }
        }
      }
    ...
    function getUserData() {
      // Get user data
      data.max = parseInt($("#berechnungContentMax").val()) || 0;
      data.min = parseInt($("#berechnungContentMin").val()) || 0;
      ...
      data.double = document.getElementById(
        "berechnungContentDoppeltBelegung"
      ).checked;
    }
    ...
    function clearCourse(course, date) {
      ...
      data.blocked[date]
        ? data.blocked[date].push(course)
        : (data.blocked[date] = [course]);

      // Get array of all wishes for sorting
      let studentWishesArray = Object.entries(
        getStudentWishes(data.schülerwünsche)
      ).reduce((current, [time, students]) => {
        Object.entries(students).forEach(([student, course]) => {
          current[student]
            ? current[student].push(course)
      ...
      data.schülerwünsche[date][course]
        .reduce((current, student) => {
          ...
          if (
            startedOverMax &&
            data.schülerwünsche[date][course].length - current.length <=
              data.max
          )
            return current;
          ...
          Object.keys(data.schülerwünsche[date])
            .sort((a, b) =>
              // Sorted courses by user data
              sortByMinMaxPush(
                data.schülerwünsche[date][a].length,
                data.schülerwünsche[date][b].length
              )
            )
            .some((alternativeCourse) => {
              ...
              if (
                ...
                (!studentWishesArray[student].includes(alternativeCourse) ||
                  data.double) &&
        ...
        .forEach((student) =>
          data.schülerwünsche[date][course].splice(
            data.schülerwünsche[date][course].indexOf(student),
            1
          )
        );
