  ...
  ["berufe", "schüler"].forEach(function (item) {
    new Table(item, function (type, self) {
      ...
      $("#" + type + "ContentImport").click(function () {
        ...
        if (path != null) {
          // Add paths to json
          settings.paths["path" + type] = settings.paths["path" + type].concat(
            path
          );
          ...
          $("#" + type + "ContentDay")
            .val(pathNames[pathNames.length - 1])
            .change();
      ...
      $("#" + type + "ContentDay").change(function (event) {
        ...
        self.load(
          excel.xlsxToArray(
            $(event.currentTarget.selectedOptions[0]).attr("path"),
            type
          ),
          type
        );
      ...
      $("#" + type + "ContentRemove").click(function () {
        ...
        settings.paths["path" + type + ""].splice(
          settings.paths["path" + type + ""].indexOf(
            $($("#" + type + "ContentDay")[0].selectedOptions[0]).attr("path")
          ),
          1
        );
        ...
        if ($("#" + type + "ContentDay")[0].childNodes.length > 0)
          self.load(
            excel.xlsxToArray(
              $($("#" + type + "ContentDay")[0].selectedOptions[0]).attr(
                "path"
              ),
              type
            ),
      ...
      function addOptions(paths) {
        return paths.map((path) => {
          ...
          var option = document
            .getElementById(type + "ContentDay")
      ...
      if ("schüler" == type)
        $("#schülerContentMerge").click(function () {
          ...
          self.load(
            settings.paths["pathschüler"].reduce((current, students) => {
              ...
              else if (current[0].equals(list.shift()))
                current = current
                  .concat([Array(current[0].length).fill("")])
                  .concat(list);
              // Error
              else {
