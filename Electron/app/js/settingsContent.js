const settings = {
  ...
  toggleDarkMode: (setVal) => {
    ...
    if (typeof setVal == "boolean")
      linking.attr("disabled", setVal ? "disabled" : null);
    else {
      ...
      // Write json
      ipc.send("writeFile", JSON.stringify(settings.settings), [
        "app",
        "json",
        "settings.json",
      ]);
    ...
    document.getElementById("settingsToggleDark").checked = settings.settings
      .darkMode
      ? true
      : false;
  },
  save: () => {
    ...
    ipc.send("writeFile", JSON.stringify(settings.paths), [
      "app",
      "json",
      "pfade.json",
    ]);
