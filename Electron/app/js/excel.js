...
const excel = {
    ...
    saveExcel: (file) => {
        ipc.send("exportFile", xlsx.build(file.map((content, index) => {
            return {
                name: new RegExp("^" + index + ". ").test(settings.current.merged[index]) ? settings.current.merged[index] : (index.toString() + ". " + settings.current.merged[index]),
                data: content
            }
        })), "Excel", ["xlsx"]);
    },
    ...
    xlsxToJSON: (xlsxPath, type) => {
        ...
        error.innerHTML = ((type == "berechnung") ? error.innerHTML : "") + "Fehler: Der Pfad <b>'" + xlsxPath + "'</b> konnte nicht gefunden werden oder ist ungültig.\n";
    ...
    xlsxToArray: (xlsxPath, type) => {
        ...
        return xlsx.reduce((current, list) => {
